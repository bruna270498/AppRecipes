...
import renderRecipeItems from '../Service/renderRecipeItems';
...
function RecipesInProgress() {
  ...
  useEffect(() => {
    ...
    if (getType === 'meals') {
      getMealByID(getId).then((response) => setRecipe(response.meals[0]));
    } else {
      getDrinkByID(getId).then((response) => setRecipe(response.drinks[0]));
    }
  }, [history]);
